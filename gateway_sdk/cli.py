...
gateway_sdk.cli
===============
Command-line interface for gateway-sdk.

Entry point registered in pyproject.toml:
    gateway-sdk = gateway_sdk.cli:main

Commands:
    gateway-sdk serve [--port PORT] [--db PATH]
        Launch the local observability dashboard.

    gateway-sdk init-db [--db PATH]
...
@click.group()
def main():
    """gateway-sdk — LLM Observability & Reliability SDK"""
    pass
...
def serve(port: int | None, db: str | None):
    ...
    click.echo(f"🚀  gateway-sdk dashboard starting at http://localhost:{effective_port}")
    click.echo("    Press Ctrl+C to stop.\n")

    uvicorn.run(
        "gateway_sdk.server.app:app",
        host="0.0.0.0",
        port=effective_port,
        reload=False,
        log_level="warning",
