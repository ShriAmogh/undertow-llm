...
import time
import pytest
from undertow_llm.rate_limiter import RedisTokenBucketLimiter, get_limiter
from undertow_llm.queue import RedisRequestQueue, get_queue

REDIS_TEST_URL = os.getenv("UNDERTOW_LLM_REDIS_URL", "redis://localhost:6379")
...
def test_redis_factory_dispatching(redis_client, monkeypatch):
    monkeypatch.setenv("UNDERTOW_LLM_REDIS_URL", REDIS_TEST_URL)
...
def test_redis_contains_no_metrics_or_cache_data(redis_client):
    ...
    """
    keys = redis_client.keys("undertow-llm:*")
