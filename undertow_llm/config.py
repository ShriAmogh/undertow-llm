...
class GatewayConfig:
    # ── Storage ──────────────────────────────────────────────────────────────
    db_path: str = field(
        default_factory=lambda: os.getenv("UNDERTOW_LLM_DB_PATH", "undertow-llm.db")
    )
    postgres_url: str | None = field(
        default_factory=lambda: os.getenv("UNDERTOW_LLM_POSTGRES_URL") or os.getenv("POSTGRES_URL")
    )
    redis_url: str | None = field(
        default_factory=lambda: os.getenv("UNDERTOW_LLM_REDIS_URL") or os.getenv("REDIS_URL")
    )
    ...
    dashboard_port: int = field(
        default_factory=lambda: int(os.getenv("UNDERTOW_LLM_DASHBOARD_PORT", "8080"))
    )
...
def configure(**kwargs) -> GatewayConfig:
    ...
    for key, value in kwargs.items():
        if not hasattr(_config, key):
            raise ValueError(
                f"Unknown undertow_llm config key: '{key}'. "
    ...
    if _config.cache_enabled:
        from undertow_llm.cache.semantic import prewarm
        prewarm()
